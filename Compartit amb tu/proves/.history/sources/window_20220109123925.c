...
char *my_return_time (int time)
{
    char Timer[5], *new_timer;
    int i = 0, x = 0;
    while (time != 0) {
        i = (time) % 10;
        time /= 10;
...
void my_runner (void)
{
    ...
    //sfText_setString(var.text, my_return_time(varscore));
    //sfText_setFont(var.text, var.font);
    //sfText_setCharacterSize(var.text, 50);
    //sfText_setColor(var.text, var.color);
    //sfText_setPosition(var.text, var.text_pos);
    my_window(&var);
